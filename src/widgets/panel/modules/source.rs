        ...
        match event.hits(cx, self.view.area()) {
            Hit::FingerScroll(se) => {
                ...
                cx.widget_action(
                    ...
                    SourceAction::VolumeChanged(self.volume),
                );
            }
            Hit::FingerDown(_) => {
                ...
                cx.widget_action(
                    self.widget_uid(),
                    &HeapLiveIdPath::default(),
                    SourceAction::MuteToggled(self.is_muted),
                );
            }
            _ => {}
        }
...
impl SourceModule {
    ...
    fn update_icon(&mut self, cx: &mut Cx) {
        let icon = if self.is_muted { "󰍭" } else { "󰍬" };
