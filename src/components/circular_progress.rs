                    ...
                    .child(
                        canvas(
                            ...
                            move |bounds, (_, percent, temp, color, temp_color), window, _cx| {
                                ...
                                // Background arc1
                                let bg1_start_pt = center
                                    + point(
                                        radius * arc1_start_rad.cos(),
                                        radius * arc1_start_rad.sin(),
                                    );
                                let bg1_end_pt = center
                                    + point(
                                        radius * arc1_end_rad.cos(),
                                        radius * arc1_end_rad.sin(),
                                    );
                                let mut bg1 = PathBuilder::default()
                                    .with_style(PathStyle::Stroke(stroke_opts));
                                ...
                                if temp > 0 {
                                    ...
                                    let fg1_end_angle =
                                        arc1_start + (arc1_end - arc1_start) * value1;
                                    let fg1_end_rad = fg1_end_angle * PI / 180.0;
                                    let fg1_end_pt = center
                                        + point(
                                            radius * fg1_end_rad.cos(),
                                            radius * fg1_end_rad.sin(),
                                        );

                                    let mut fg1 = PathBuilder::default()
                                        .with_style(PathStyle::Stroke(stroke_opts));
                                    fg1.move_to(bg1_start_pt);
                                    fg1.arc_to(
                                        point(radius, radius),
                                        px(0.0),
                                        (fg1_end_rad - arc1_start_rad) > PI,
                                        true,
                                        fg1_end_pt,
                                    );
                                ...
                                // Background arc2
                                let bg2_start_pt = center
                                    + point(
                                        radius * arc2_start_rad.cos(),
                                        radius * arc2_start_rad.sin(),
                                    );
                                let bg2_end_pt = center
                                    + point(
                                        radius * arc2_end_rad.cos(),
                                        radius * arc2_end_rad.sin(),
                                    );
                                let mut bg2 = PathBuilder::default()
                                    .with_style(PathStyle::Stroke(stroke_opts));
                                ...
                                if percent > 0 {
                                    ...
                                    let fg2_end_angle =
                                        arc2_start + (arc2_end - arc2_start) * value2;
                                    let fg2_end_rad = fg2_end_angle * PI / 180.0;
                                    let fg2_end_pt = center
                                        + point(
                                            radius * fg2_end_rad.cos(),
                                            radius * fg2_end_rad.sin(),
                                        );

                                    let mut fg2 = PathBuilder::default()
                                        .with_style(PathStyle::Stroke(stroke_opts));
                                    fg2.move_to(bg2_start_pt);
                                    fg2.arc_to(
                                        point(radius, radius),
                                        px(0.0),
                                        (fg2_end_rad - arc2_start_rad) > PI,
                                        true,
                                        fg2_end_pt,
                                    );
